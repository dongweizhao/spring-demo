...
public class HttpTraceLogFilter extends OncePerRequestFilter implements Ordered {
     ...
     * 比如健康检查、swagger
     */
    @Value("#{'${http.ignorePrintUriList:/health,/swagger-ui,/favicon,/v2/api-docs,/webjars/springfox-swagger-ui}'.split(',')}")
    protected List<String> ignorePrintUriList;
    /**
     * 如果uri是如下列表中的一项开头的，那么request不打印
     * 比如文件上传相关的，那就request不打印，但是打印response
     */
    @Value("#{'${http.ignorePrintRequestUriList:}'.split(',')}")
    protected List<String> ignorePrintRequestUriList;
    /**
     * 如果uri是如下列表中的一项开头的，那么response不打印
     * 比如文章相关的，就是打印request不打印response，比如针对一些返回私钥的接口也需要配置在这里，不打印私钥信息
     */
    @Value("#{'${http.ignorePrintResponseUriList:}'.split(',')}")
    protected List<String> ignorePrintResponseUriList;

    /**
     * 需要脱敏的header头的key，在打印头的时候进行脱敏
     * 不区分大小写
     */
    protected List<String> desenHeaderList =new ArrayList<>();
    ...
    private String desenData(String data) {
        int len = data.length();
        Integer totalShowDesenCharNumber = showDesenCharNumber * 2;
